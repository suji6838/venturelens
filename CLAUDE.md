...
- 한글이 Windows 환경에서 curl로 보낼 때 깨져서(인코딩 문제) 알림 제목/내용은 영문("VentureLens - Claude Code done")으로 고정되어 있음
- 다른 프로젝트(온담채-웰니스, 요즘트렌드, SaveLoop, 별빛타로)에도 같은 토픽으로 동일하게 설정되어 있어, 어느 프로젝트가 끝났는지는 알림 제목으로 구분
