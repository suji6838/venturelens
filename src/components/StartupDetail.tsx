...
export default function StartupDetail({ startup }: Props) {
  ...
  return <section className="detail" aria-live="polite">
    <div className="detail-head"><div><p className="eyebrow">AI INVESTMENT BRIEF</p><h2>{startup.name}</h2><p className="detail-summary">{startup.summary}</p>{startup.sourceUrl && <p className="detail-links"><a href={startup.sourceUrl} target="_blank" rel="noopener noreferrer" className="source-link">근거 기사 보기 →</a></p>}</div><div className="detail-score"><b>{startup.score}</b><span>투자매력도</span></div></div>
